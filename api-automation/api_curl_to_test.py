...
使用方法:
    python api_curl_to_test.py "curl --location 'https://xxx.com/api/path' --header 'Content-Type: application/json' --data '{...}'"
...
def build_html_report(api_info: Dict[str, str], cases: List[Dict], test_results: Dict[str, str],
                      response_details: Optional[Dict[str, Dict[str, str]]] = None) -> str:
    """
    生成 HTML 表格报告文件，返回文件路径。
    """
    api_name = api_info["api_name"]
    reports_dir = BASE_DIR / "reports"
    reports_dir.mkdir(exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    html_path = reports_dir / f"result_{timestamp}.html"
    ...
    for case in cases:
        ...
        response_body = response_body.replace("\n", "").replace("\r", "")
        # HTML 转义
        body_str = html_mod.escape(body_str)
        message = html_mod.escape(message)
        response_body = html_mod.escape(response_body)

        rows_html += f"""        <tr>
          <td>{api_name}</td>
          <td>{html_mod.escape(title)}</td>
          <td class="body-cell" title="{body_str}">{body_str[:80]}{'...' if len(body_str) > 80 else ''}</td>
          <td>{http_status}</td>
          <td>{biz_code}</td>
          <td>{message}</td>
          <td class="{result_class}">{result_display}</td>
          <td class="body-cell" title="{response_body}">{response_body[:80]}{'...' if len(response_body) > 80 else ''}</td>
        </tr>\n"""
    ...
    rows_html += f"""        <tr>
          ...
          <td>-</td>
          <td>-</td>
          <td class="{rt_class}">{rt_display}</td>
          <td>-</td>
        </tr>\n"""
...
<head>
    ...
    <style>
        ...
        .total {{ color: #333; font-weight: bold; }}
        .pass-count {{ color: #52c41a; font-weight: bold; }}
        .fail-count {{ color: #f5222d; font-weight: bold; }}
        ...
        th {{ background: #4a90d9; color: #fff; padding: 12px 10px; text-align: left; font-size: 13px; white-space: nowrap; }}
        td {{ padding: 10px; border-bottom: 1px solid #eee; font-size: 13px; word-break: break-all; }}
        tr:hover {{ background: #f0f7ff; }}
        .pass {{ color: #52c41a; font-weight: bold; }}
        .fail {{ color: #f5222d; font-weight: bold; }}
        .skip {{ color: #faad14; font-weight: bold; }}
        .body-cell {{ max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }}
        .body-cell:hover {{ white-space: normal; word-break: break-all; }}
        .footer {{ margin-top: 20px; color: #999; font-size: 12px; }}
    </style>
</head>
<body>
    <h1>📊 接口测试报告 - {api_name}</h1>
    <div class="summary">
        <span class="total">总计: {total_count} 条用例</span>
        <span class="pass-count">✅ 通过: {passed_count}</span>
        <span class="fail-count">❌ 失败: {failed_count}</span>
...
{rows_html}      </tbody>
    ...
    <div class="footer">由 curl_to_test.py 自动生成 | {gen_time}</div>
</body>
</html>"""
...
示例:
    ...
    if not args.no_run:
        ...
        # 生成 HTML 表格报告
        html_report_path = build_html_report(api_info, cases, test_results, response_details)

        # 生成 Excel 报告
        xlsx_report_path = build_xlsx_report(api_info, cases, test_results, response_details)
        ...
        print(f"\n📁 测试报告: {report_path}")
        print(f"📊 HTML表格报告: {html_report_path}")
        print(f"📊 Excel报告: {xlsx_report_path}")
    ...
    if not args.no_run:
        ...
        print(f"   HTML表格: {html_report_path}")
        print(f"   Excel表格: {xlsx_report_path}")
    print(f"{'=' * 60}\n")
